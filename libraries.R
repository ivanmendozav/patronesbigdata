...
getData <- function(limit=100000){
  ...
  query <- paste("select * from locations where recorded_at between '2019-09-01' and '2020-01-01' limit ",limit, sep="")
...
getAttractors <- function(tripdata, eps=25, minPts=10){
  ...
  plotmap(min(wgscent$x)-0.002,max(wgscent$x)+0.002,min(wgscent$y)-0.007,max(wgscent$y)+0.007,wgscent)
  return (tripdata)
}

plotmap <- function(lon1,lon2,lat1,lat2,points,projection="+init=epsg:4326"){
